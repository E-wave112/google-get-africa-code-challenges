...
class CarTestVocabulary(unittest.TestCase):
    def test_car_vocab(self):
        self.assertEquals(
            find_shortest_word(
                "RT 123 SO", ["car", "rest", "rust", "rot", "sir", "cast"]
            ),
            "sort",
        )
        self.assertEqual(
            find_shortest_word(
                "RC 10014", ["sort", "car", "rest", "rust", "sir", "cast"]
            ),
            "car",
        )
        self.assertEqual(
            find_shortest_word("Ps 3S1t0", ["step", "steps", "stripe", "stepple"]),
            "steps",
        )
        self.assertEqual(
            find_shortest_word("ST 123 CA", ["cat", "act", "sat", "cast", "cast"]),
            "cast",
        )
